...
- **`api-local` is skipped in CI and excluded from all other projects — requires Docker** — the `api-local` project is wrapped in a `process.env.CI` guard in `playwright.config.ts`. Additionally, `**/api-inventory/**` is added to `testIgnore` on every other project (`main-desktop-*`, `regression`, `staging`, `production`) to prevent them from picking up the specs and hitting `localhost:8080`. Run api-inventory locally with the `taqelah/playpi` Docker container active and the RESTful Inventory Manager started on port 8080. The specs use the `request` fixture and hit real endpoints — do not revert them to `page.route()` mocks.
