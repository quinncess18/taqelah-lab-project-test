...
- **`staging`/`production` pick up all specs — new files are auto-included** — neither has a `testDir` restriction. Both ignore `**/*.visual.spec.ts` and `**/api-inventory/**`. Any new spec file added outside those paths will automatically run in both environment projects.
- **`api-local` requires Docker and is excluded everywhere except locally** — the project is wrapped in a `process.env.CI` guard and `**/api-inventory/**` is in `testIgnore` on all other projects. Run it locally with the `taqelah/playpi` container active and the RESTful Inventory Manager started on port 8080. Specs use the `request` fixture — do not revert to `page.route()` mocks.
